...
def NODEGUARDrequest (cif,node,state=0,result=True):
	cmsg = ntcan.CMSG()
	print("Nodeguard RTR %d -> " % node, end=' ')
	...
	def monitor (self,node,emerg_error_code=0,error_reg=0,no_of_errors=0,manu_specific_error_field=0,result=True):
		...
		try: 
			...
			if self.cmsg.len == 8:
				...
				print("EMERGENCYmonitor node: %d -> " %(node), end=' ')
...
def SDOread (cif,node,index,subindex,data=0,result=True):
	...
	print("SDOread node: %d index: 0x%0.4X subindex: 0x%0.2X -> " %(node,index,subindex), end=' ')
...
def SDOwrite1Byte (cif,node,index,subindex,data,result=True):
	...
	rdindex = 0
	cif.canIdAdd(SDOTX_ID+node)
	print("SDOwrite1Byte node: %d index: 0x%0.4X subindex: 0x%0.2X data: 0x%0.2X -> " %(node,index,subindex,data), end=' ')
...
def SDOwrite2Byte (cif,node,index,subindex,data,result=True):
	...
	rdindex = 0
	cif.canIdAdd(SDOTX_ID+node)
	print("SDOwrite2Byte node: %d index: 0x%0.4X subindex: 0x%0.2X data: 0x%0.4X -> " %(node,index,subindex,data), end=' ')
...
def SDOwrite4Byte (cif,node,index,subindex,data,result=True):
	...
	rdindex = 0
	cif.canIdAdd(SDOTX_ID+node)
	print("SDOwrite4Byte node: %d index: 0x%0.4X subindex: 0x%0.2X data: 0x%0.8X -> " %(node,index,subindex,data), end=' ')
...
def SDOwriteString (cif,node,index,subindex,str,data=0,result=True):
	...
	rdindex = 0
	cif.canIdAdd(SDOTX_ID+node)
	print("SDOwriteString node: %d index: 0x%0.4X subindex: 0x%0.2X data: %s -> " %(node,index,subindex,str), end=' ')
...
class SDO:
			...
			if self.cmsg.len == 8:
				...
				if (rdindex == index and self.cmsg.data.c[3] == subindex):
					if self.cmsg.data.c[0] == SDO_SEG_ABORT_TRANSFER:
						...
						print("data: %d  0x%0.4X" % (data,data), end=' ')
						if (self.cmsg.data.c[4] >= 0x20): print("%c" % self.cmsg.data.c[4], end=' ')
						else: print(" ", end=' ')
						...
						if (self.cmsg.data.c[5] >= 0x20): print("%c" % self.cmsg.data.c[5], end=' ')
						else: print(" ", end=' ')
						...
						if (self.cmsg.data.c[6] >= 0x20): print("%c" % self.cmsg.data.c[6], end=' ')
						else: print(" ", end=' ')
	...
	#		print self.cmsg 
		...
		except IOError as xxx_todo_changeme13:
			(errno) = xxx_todo_changeme13
			print("I/O error(%s): " % (errno))
			result=False
		self.cif.canIdDelete(SDOTX_ID+node)
		return result,data
	def writeString (self,node,index,subindex,str,data=0,result=True):
		...
		if len(str) == 0 or len(str) > 4:
			...
			except IOError as xxx_todo_changeme14:
				(errno) = xxx_todo_changeme14
				print("I/O error(%s): " % (errno))
				result=False
		self.cif.canIdDelete(SDOTX_ID+node)
		return result,data
	...
	def scan (self,node,startindex=0,stopindex=0,data=0,result=True):
		...
		#							print "error: 0x%0.8X %s" % (self.cmsg.data.l[1],SDOerror.get(self.cmsg.data.l[1]))
								...
								data = self.cmsg.data.l[1]
								print("SDOscan node: %d index: 0x%0.4X subindex: 0x%0.2X -> " %(node,index,subindex), end=' ')
								print("data: %d  0x%0.8X" % (data,data))
							elif self.cmsg.data.c[0] == SDO_SEG_RES_INIT_UPLOAD_1BYTE:      
								data = self.cmsg.data.l[1] & 0xFF
								print("SDOscan node: %d index: 0x%0.4X subindex: 0x%0.2X -> " %(node,index,subindex), end=' ')
									...
									try: 
										...
										if ((self.cmsg.data.c[0]&0x10) != (cmd&0x10)):
											...
											if (self.cmsg.data.c[0] & 0x01): 		# last data
												...
												for i in range(7):
													lstr.append(chr(self.cmsg.data.c[i+1]))
									except IOError as xxx_todo_changeme15:
											(errno) = xxx_todo_changeme15
											print("I/O error(%s): " % (errno))
											result = False
									if cmd==0x60: cmd=0x70
									else: cmd=0x60
			...
			#			print self.cif.msg_count 
						...
						print("unknown")
						result = False
				except IOError as xxx_todo_changeme16:
					(errno) = xxx_todo_changeme16
					print("I/O error(%s): " % (errno))
					result = False
					return result,data
				subindex = subindex + 1	
...
def PDO1writeByte (cif,node,len,d0=0,d1=0,d2=0,d3=0,d4=0,d5=0,d6=0,d7=0):
	cmsg = ntcan.CMSG()
	print("PDO1writeByte node: %d -> " %(node), end=' ')
...
def PDO1writeShort (cif,node,len,d0=0,d1=0,d2=0,d3=0):
	cmsg = ntcan.CMSG()
	print("PDO1writeShort node: %d -> " %(node), end=' ')
...
def PDO1writeLong (cif,node,len,d0=0,d1=0):
	cmsg = ntcan.CMSG()
	print("PDO1writeLong node: %d -> " %(node), end=' ')
...
def PDO2writeByte (cif,node,len,d0=0,d1=0,d2=0,d3=0,d4=0,d5=0,d6=0,d7=0):
	cmsg = ntcan.CMSG()
	print("PDO2writeByte node: %d -> " %(node), end=' ')
...
def PDO2writeShort (cif,node,len,d0=0,d1=0,d2=0,d3=0):
	cmsg = ntcan.CMSG()
	print("PDO2writeShort node: %d -> " %(node), end=' ')
	cmsg.canWriteShort(cif, PDO1RX_ID+node,len,d0,d1,d2,d3)
	print(cmsg)
	del cmsg
...
def PDO2writeLong (cif,node,len,d0=0,d1=0):
	cmsg = ntcan.CMSG()
	print("PDO2writeLong node: %d -> " %(node), end=' ')
...
def PDO3writeByte (cif,node,len,d0=0,d1=0,d2=0,d3=0,d4=0,d5=0,d6=0,d7=0):
	cmsg = ntcan.CMSG()
	print("PDO3writeByte node: %d -> " %(node), end=' ')
...
def PDO3writeShort (cif,node,len,d0=0,d1=0,d2=0,d3=0):
	cmsg = ntcan.CMSG()
	print("PDO3writeShort node: %d -> " %(node), end=' ')
...
def PDO3writeLong (cif,node,len,d0=0,d1=0):
	cmsg = ntcan.CMSG()
	print("PDO3writeLong node: %d -> " %(node), end=' ')
...
def PDO4writeByte (cif,node,len,d0=0,d1=0,d2=0,d3=0,d4=0,d5=0,d6=0,d7=0):
	cmsg = ntcan.CMSG()
	print("PDO4writeByte node: %d -> " %(node), end=' ')
...
def PDO4writeShort (cif,node,len,d0=0,d1=0,d2=0,d3=0):
	cmsg = ntcan.CMSG()
	print("PDO4writeShort node: %d -> " %(node), end=' ')
...
def PDO4writeLong (cif,node,len,d0=0,d1=0):
	cmsg = ntcan.CMSG()
	print("PDO4writeLong node: %d -> " %(node), end=' ')
...
def PDO1read (cif,node,cmsg=0,result=True):
	...
	print("PDO1read node: %d -> " %(node), end=' ')
	lcmsg.canWriteByte(cif, PDO1TX_ID+node,0x10)
	try: 
		lcmsg.canRead(cif)
		print(lcmsg) 
	except IOError as xxx_todo_changeme17:
		    (errno) = xxx_todo_changeme17
		    print("I/O error(%s): " % (errno))
		    result = False
...
def PDO2read (cif,node,cmsg=0,result=True):
	...
	print("PDO2read node: %d -> " %(node), end=' ')
	lcmsg.canWriteByte(cif, PDO2TX_ID+node,0x10)
	try: 
		lcmsg.canRead(cif)
		print(lcmsg) 
	except IOError as xxx_todo_changeme18:
		    (errno) = xxx_todo_changeme18
		    print("I/O error(%s): " % (errno))
		    result = False
...
def PDO3read (cif,node,cmsg=0,result=True):
	...
	print("PDO3read node: %d -> " %(node), end=' ')
	lcmsg.canWriteByte(cif, PDO3TX_ID+node,0x10)
	try: 
		lcmsg.canRead(cif)
		print(lcmsg) 
	except IOError as xxx_todo_changeme19:
		    (errno) = xxx_todo_changeme19
		    print("I/O error(%s): " % (errno))
		    result = False
...
def PDO4read (cif,node,cmsg=0,result=True):
	...
	print("PDO4read node: %d -> " %(node), end=' ')
	lcmsg.canWriteByte(cif, PDO4TX_ID+node,0x10)
	try: 
		lcmsg.canRead(cif)
		print(lcmsg) 
	except IOError as xxx_todo_changeme20:
		    (errno) = xxx_todo_changeme20
		    print("I/O error(%s): " % (errno))
		    result = False
...
class PDO:
	...
	def write (self,node,d0=0,l0=0,d1=0,l1=0,d2=0,l2=0,d3=0,l3=0,d4=0,l4=0,d5=0,l5=0,d6=0,l6=0,d7=0,l7=0):
		print("PDO%dwrite node: %d -> " %(self.obj,node), end=' ')
...
#				print _candata
	...
	def writeByte (self,node,len,d0=0,d1=0,d2=0,d3=0,d4=0,d5=0,d6=0,d7=0):
		print("PDO%dwriteByte node: %d -> " %(self.obj,node), end=' ')
	...
	def read (self,node,cmsg=0,result=True,silent=False):
		...
		if (silent): print("PDO%dread node: %d -> " %(self.obj,node), end=' ')
		self.cmsg.canWriteByte(self.cif,(self.txid+node),0x10)    #rtr
		try: 
			self.cmsg.canRead(self.cif)
			if (silent): print(cmsg) 
		except IOError as xxx_todo_changeme21:
			    (errno) = xxx_todo_changeme21
	...
	def readNoRtr (self,node,cmsg=0,result=True,silent=False):
		self.cif.canIdAdd(self.txid+node)
		if (silent): print("PDO%dreadNoRtr node: %d -> " %(self.obj,node), end=' ')
		#self.cmsg.canWriteByte(self.cif,(self.txid+node),0x10)    #rtr
		try: 
			self.cmsg.canRead(self.cif)
			if (silent): print(cmsg) 
		except IOError as xxx_todo_changeme22:
			    (errno) = xxx_todo_changeme22
			    if (silent): print("I/O error(%s): " % (errno))
			    result = False
		self.cif.canIdDelete(self.txid+node)
		cmsg = self.cmsg
