...
static void prvSetupTimerInterrupt(void)
{
	...
	#if( configTICK_USE_TC==1 )
	{
		INTC_register_interrupt(&vTick, AVR32_TC1_IRQ0, 1);
		INTC_register_interrupt(&vTick, AVR32_TC1_IRQ1, 1);
		INTC_register_interrupt(&vTick, AVR32_TC1_IRQ2, 1);
		...
		tc_write_rc( tc, 1, ( configPBA_CLOCK_HZ/2) / configTICK_RATE_HZ );

		tc_configure_interrupts( tc, 1, &tc_interrupt );
	...
	#else
	{
		INTC_register_interrupt(&vTick, AVR32_CORE_COMPARE_IRQ, 0);
